...
std::list<Node> DLS(const Problem& problem, Node node, float depth, bool& limit_reached){
    ...
    for (auto action : problem.get_actions(node.get_state())){
        ...
        result = DLS(problem, child, depth-0.5, limit_reached);
        if(!result.empty()){
            break;
        }
    }
...
std::list<Node> IDDFS(const Problem& problem){
    ...
    while(true){
        ...
        result = DLS(problem, Node(problem.get_initial(), 0.0), depth, limit_reached);
...
int main(){
    std::vector<std::vector<char> > map_matrix = read_file_to_vector("../maps/map4.map");
